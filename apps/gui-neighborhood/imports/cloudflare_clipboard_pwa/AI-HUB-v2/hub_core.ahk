...
ExportChatMarkdown(*) {
    ...
    vaultPath := "O:\_Theophysics_v4\04_THEOPYHISCS\AI_CHAT_LOGS"
    if DirExist("O:\_Theophysics_v4\04_THEOPYHISCS") {
...
PersistSysPrompts() {
    ...
    jsonStr .= "`n]"
    tmpFile := SYSPROMPT_FILE ".tmp"
    try FileDelete(tmpFile)
    FileAppend(jsonStr, tmpFile, "UTF-8")
    try FileDelete(SYSPROMPT_FILE)
    FileMove(tmpFile, SYSPROMPT_FILE)
}
...
AddDataEntry(*) {
    global gShell, gStoredData, gEditingDataIndex
    ...
    if gEditingDataIndex > 0 && gEditingDataIndex <= gStoredData.Length {
        ; Update existing entry in-place (preserves original ID and created date)
        existing := gStoredData[gEditingDataIndex]
        existing.category := cat
        existing.name := name
        existing.value := value
        existing.tags := tags
        existing.modified := FormatTime(A_Now, "yyyy-MM-dd HH:mm")
        gEditingDataIndex := 0
    } else {
        ; Create new entry
        entry := {
...
; Maps displayed ListView row → actual gStoredData index
global gDataRowMap := []

RefreshDataList(*) {
    global gShell, gStoredData, gDataRowMap
    gShell.dataLV.Delete()
    gDataRowMap := []
    filterCat := gShell.dataFilterDDL.Text
    searchTerm := gShell.dataSearchEdit.Value
    for idx, entry in gStoredData {
...
; Resolve a ListView display row to the real gStoredData index
_DataRealIndex(displayRow) {
    global gDataRowMap
    if displayRow > 0 && displayRow <= gDataRowMap.Length
        return gDataRowMap[displayRow]
    return 0
}

CopyDataValue(lv, row) {
    global gStoredData
    realIdx := _DataRealIndex(row)
    if realIdx > 0 && realIdx <= gStoredData.Length {
        A_Clipboard := gStoredData[realIdx].value
        ToolTip("Copied!")
        SetTimer(() => ToolTip(), -1500)
    }
}
...
; Track which entry is being edited so we update in-place instead of deleting
global gEditingDataIndex := 0
...
EditDataEntry(*) {
    ...
    ; Remember which entry we're editing — DON'T remove it yet
    gEditingDataIndex := realIdx
}
...
Load_All() {
    ...
    ; Only seed defaults on truly first run (neither file exists at all)
    ; NEVER overwrite existing files — a failed read is not the same as "no data"
    if gItems.Length = 0 && !FileExist(HOTKEY_FILE) && !FileExist(HOTSTR_FILE) {
...
Save_All() {
    ...
    ; Write to temp files first, then rename — prevents data loss on crash
    hkTmp := HOTKEY_FILE ".tmp"
    hsTmp := HOTSTR_FILE ".tmp"
    try FileDelete(hkTmp)
    try FileDelete(hsTmp)
    FileAppend(hk, hkTmp, "UTF-8")
    FileAppend(hs, hsTmp, "UTF-8")
    try FileDelete(HOTKEY_FILE)
    try FileDelete(HOTSTR_FILE)
    FileMove(hkTmp, HOTKEY_FILE)
    FileMove(hsTmp, HOTSTR_FILE)
}
