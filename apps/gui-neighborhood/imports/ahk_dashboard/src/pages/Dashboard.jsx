...
import HubCapturePanel from '../components/HubCapturePanel'
import HubActivityPanel from '../components/HubActivityPanel'
import HubAskPanel from '../components/HubAskPanel'
...
function buildLocalReport(projects, roadmap, overview) {
  const completedTasks = roadmap.filter(task => task.status === 'completed')
  const openTasks = roadmap.filter(task => task.status !== 'completed')
  const priorityTasks = openTasks
    .filter(task => task.priority === 'high')
    .slice(0, 5)

  const title = `Strategic Dashboard Snapshot - ${new Date().toLocaleDateString()}`
  const summary = `${projects.length} projects tracked, ${completedTasks.length}/${roadmap.length} tasks complete, projected ROI ${overview.projectedROI}.`
  const content = `# ${title}

## Executive Summary

- Active projects: ${overview.activeProjects}
- Completed tasks: ${completedTasks.length}/${roadmap.length}
- Projected ROI: ${overview.projectedROI}
- Total budget: ${overview.totalBudget}

## Priority Tasks

${priorityTasks.length > 0
    ? priorityTasks.map(task => `- ${task.title} (${task.project || 'General'})`).join('\n')
    : '- No high-priority tasks are currently open.'}

## Active Projects

${projects.map(project => `- ${project.name}: ${project.progress}% complete, next milestone ${project.nextMilestone}`).join('\n')}
`
...
export default function Dashboard() {
  ...
  async function handleGenerateReport() {
    ...
    if (!ENABLE_BACKEND_FEATURES) {
      const localReport = buildLocalReport(projects, roadmap, overview)
      const savedReport = ReportsManager.saveReport({
      ...
      if (savedReport) {
        setMessage('Report saved locally for Cloudflare mode ✅')
        setMessageType('success')
        setTimeout(() => setMessage(''), 8000)
      } else {
        setMessage('Failed to save local report ❌')
...
`;
          ...
          if (saveData.success) {
            console.log('💾 Report saved to archive:', saveData.filename);
            setMessage(`Report ready 🎉 - ${data.report.sections.length} sections, Saved to archive ✅`);
          } else {
            console.warn('⚠️ Report generated but not saved:', saveData.error);
            setMessage(`Report ready 🎉 - ${data.report.sections.length} sections (archive save failed)`);
          }
        } catch (saveError) {
  ...
  const handleHubCaptured = (result) => {
    setHubRefreshKey(prev => prev + 1)
    setMessage(
      result.mode === 'cloudflare'
        ? 'Hub event captured for the team ✅'
        : 'Hub capture saved locally until Cloudflare bindings are live ⚠️'
    )
    setMessageType(result.mode === 'cloudflare' ? 'success' : 'error')
    setTimeout(() => setMessage(''), 8000)
  }
  ...
  async function handleRunAnalysis() {
    ...
    if (!ENABLE_BACKEND_FEATURES) {
      ...
      setMessage('Analysis prepared in Emma AI CoPilot ✅')
      setMessageType('success')
      setTimeout(() => setMessage(''), 8000)
  ...
  return (
    <div className="space-y-8 page-transition">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <HubCapturePanel onCaptured={handleHubCaptured} />
        <HubActivityPanel refreshKey={hubRefreshKey} />
      </div>

      <HubAskPanel refreshKey={hubRefreshKey} />
    </div>
  )
}
