...
; ============================================================
; AI-HUB v2 â€” Dark Mode Hotkeys, Hotstrings & AI Assistant
; ============================================================
...
;   Ctrl+Shift+Alt+P = Launch Startup Launcher
; ============================================================
...
global gSavedW := ""              ; saved window width
global gSavedH := ""              ; saved window height
...
Hub_Boot() {
    ...
    ; Start clipboard sync bridge (auto-saves clipboard to local server)
    bridgePath := A_ScriptDir "\clipsync-bridge\clipsync_bridge.ahk"
    if FileExist(bridgePath) {
        DetectHiddenWindows(true)
        if !WinExist("clipsync_bridge.ahk ahk_class AutoHotkey")
            try Run(bridgePath)
        DetectHiddenWindows(false)
    }

    ; Start FIS (File Intelligence System) â€” always-on file watcher + crawler
    fisBat := A_ScriptDir "\..\START_FIS.bat"
    if FileExist(fisBat)
        try Run(fisBat, "", "Hide")
...
CreateMainGUI() {
    ...
    gShell.gui := Gui("+Resize", "AI-HUB v2 â€” Hotkeys, Hotstrings & AI")
    ...
    ; Build tabs dynamically â€” NO bold on tab headers (7 tabs overflow with bold)
...
SetupTray() {
    ...
    A_TrayMenu.Add()
    A_TrayMenu.Add("Quick Actions", (*) => ShowQuickActionPopup())
    A_TrayMenu.Add("Clipboard", (*) => LaunchHtmlPanel("http://127.0.0.1:3456/clipboard3", "POF-Clipboard"))
    A_TrayMenu.Add("Prompts", (*) => LaunchHtmlPanel("http://127.0.0.1:3456/prompts", "POF-Prompts"))
    A_TrayMenu.Add("Links", (*) => LaunchHtmlPanel("http://127.0.0.1:3456/links", "POF-Links"))
    A_TrayMenu.Add("Popup Hub", (*) => Run('"D:\DONT TOUCH BOOT UP\PopupHub\START_POPUP_HUB.bat"'))
...
SaveGuiPos() {
    global gShell, gSavedX, gSavedY, gSavedW, gSavedH
    if !IsObject(gShell)
        return
    try {
        if !gShell.HasOwnProp("gui")
            return
        hwnd := gShell.gui.Hwnd
    } catch
        return
    if WinExist("ahk_id " hwnd) {
        WinGetPos(&x, &y, &w, &h, "ahk_id " hwnd)
        ; Skip minimized coordinates (-32000) â€” Windows parks minimized windows there
        if (x > -10000 && y > -10000 && w > 0 && h > 0) {
            gSavedX := x
            gSavedY := y
            gSavedW := w
            gSavedH := h
...
IsManagedHtmlPanel(hwnd := 0) {
    ...
    try exe := WinGetProcessName("ahk_id " hwnd)
        ...
        return false
    if !(exe = "msedge.exe" || exe = "chrome.exe")
        return false
    for , needle in ["POF 2828", "Task Calendar", "Deepcrawl", "n8n", "Search"] {
...
SaveActiveHtmlPanelState() {
    hwnd := WinGetID("A")
    if !hwnd
        return false
    if !IsManagedHtmlPanel(hwnd)
        return false
    ...
    pinned := (exStyle & 0x8) ? "true" : "false"

    stateFile := A_ScriptDir "\Data\window_state.json"
    SplitPath(stateFile, , &stateDir)
    if !DirExist(stateDir)
        DirCreate(stateDir)

    json := "{`n"
    json .= '  "position": {' "`n"
    json .= '    "title": ' Dq(title) "," "`n"
    json .= '    "x": ' x "," "`n"
    json .= '    "y": ' y "," "`n"
    json .= '    "w": ' w "," "`n"
    json .= '    "h": ' h "`n"
    ...
    json .= '  "pin": {' "`n"
    json .= '    "title": ' Dq(title) "," "`n"
    json .= '    "pinned": ' pinned "`n"
    json .= '  }' "`n"
    json .= "}`n"
    FileDelete(stateFile)
    FileAppend(json, stateFile, "UTF-8")
    return true
}

ShowGui() {
    global gShell, gRememberPos, gSavedX, gSavedY, gSavedW, gSavedH
    if !IsObject(gShell)
        return
    try {
        if !gShell.HasOwnProp("gui")
        ...
        return
    if gRememberPos && (gSavedX != "" || gSavedY != "" || gSavedW != "" || gSavedH != "")
        guiObj.Show(FormatSavedGuiPlacement())
    else
        guiObj.Show()
}
...
ToggleAlwaysOnTop() {
    ...
    try WinSetAlwaysOnTop(gAlwaysOnTop ? 1 : 0, "ahk_id " gShell.gui.Hwnd)
...
ToggleRememberPos() {
    ...
    ; Sync Utilities tab checkbox if it exists
    try {
        if gShell.HasOwnProp("chkRememberPos") {
...
SaveUtilityState() {
    ...
    global gSavedX, gSavedY, gSavedW, gSavedH
    try {
        ...
        IniWrite(gSavedW, CONFIG_FILE, "Utilities", "savedW")
        IniWrite(gSavedH, CONFIG_FILE, "Utilities", "savedH")
    }
}
...
FormatSavedGuiPlacement() {
    global gSavedX, gSavedY, gSavedW, gSavedH
    opts := ""
    if (gSavedX != "" && gSavedY != "" && gSavedX > -10000 && gSavedY > -10000)
        opts .= "x" gSavedX " y" gSavedY " "
    if (gSavedW != "" && gSavedH != "" && gSavedW > 0 && gSavedH > 0)
        opts .= "w" gSavedW " h" gSavedH
    return Trim(opts)
}
...
BuildPromptsTab() {
    ...
    gShell.gui.Add("Text", "x+10 c888888", "(name + /slash trigger, e.g. fix â†’ /fix)")
...
; Handler for /slash shortcuts â€” pastes the prompt template at cursor
...
; ============================================================
; CHAT EXPORT â€” Save conversation as Markdown
; ============================================================
ExportChatMarkdown(*) {
    global gShell, gChatHistory
    ...
    vaultPath := "O:\_Theophysics_v3\04_THEOPHYSICS\AI_CHAT_LOGS"
    if DirExist("O:\_Theophysics_v3\04_THEOPHYSICS") {
...
; ============================================================
; SEND PROMPT TO CHAT â€” Load prompt template into chat input
; ============================================================
SendPromptToChat(*) {
    global gShell, gPrompts, gEditingPromptIndex
    ...
    ToolTip("Loaded '" p.name "' into chat â€” edit and send")
    SetTimer(() => ToolTip(), -2000)
}
...
SaveSettings(*) {
    ...
    content .= "savedW=" gSavedW "`n"
    content .= "savedH=" gSavedH "`n"
...
LoadSettings() {
    ...
    try {
        ...
        gSavedW := IniRead(CONFIG_FILE, "Utilities", "savedW", "")
        gSavedH := IniRead(CONFIG_FILE, "Utilities", "savedH", "")
...
CallOpenAI(prompt, history, sysPrompt := "") {
    ...
    if model = ""
        model := "gpt-4o-mini"
    if apiKey = ""
        return "Please configure your OpenAI API key in Settings or OPENAI_API_KEY."
...
CallClaude(prompt, history, sysPrompt := "") {
    ...
    if model = ""
        model := "claude-sonnet-4-20250514"
    if apiKey = ""
        return "Please configure your Claude API key in Settings or ANTHROPIC_API_KEY."
...
SmartFixActiveWindow(*) {
    ...
    ToolTip("Fixing text...")
    response := CallAI(BuildSmartFixPrompt(textToFix), [])
    ToolTip()

    if SmartFixIsError(response) {
        MsgBox(response, "Smart Fix Error", "Icon!")
        return
    }

    response := SmartFixCleanResponse(response)
    if response = "" {
...
SmartFixCleanResponse(response) {
...
^!y:: {
    if SaveActiveHtmlPanelState() {
        ToolTip("Panel position saved")
        SetTimer(() => ToolTip(), -1500)
        return
    }
    ToggleRememberPos()
...
ShowQuickActionPopup() {
    ...
    popGui.Add("Text", "xm w200", "âš¡ QUICK ACTIONS  (Esc to close)")
    ...
    for i, b in builtins {
        if b.name = "To Clipboard" {
            popGui.Add("Text", "xm w200 h1 Background2a2a2a y+4")
            btn := popGui.Add("Button", "xm w200 h28", "ðŸ“‹ Copy Selection")
...
^Space::SmartFixActiveWindow()


; ============================================================
; HTML PANEL HOTKEYS â€” Ctrl+Alt+C/P/K/M
...
LaunchHtmlPanel(url, title) {
    ; Map short keys to the specific app-window titles we own.
    ; This avoids collisions with other tools that happen to include
    ; words like "Clipboard" or "Calendar" in their window titles.
    static titleMap := Map(
        "POF-Clipboard", ["POF 2828 — Clipboard", "POF 2828 - Clipboard", "POF 2828 â€” Clipboard"],
        "POF-Prompts",   ["POF 2828 — Prompt Picker", "POF 2828 Prompt Picker"],
        "POF-Links",     ["POF 2828 — Research Links", "POF 2828 â€” Research Links"],
        "POF-Calendar",  ["Task Calendar · POF 2828", "Task Calendar Â· POF 2828"],
        "POF-Deepcrawl", ["Deepcrawl"],
        "POF-n8n",       ["n8n"],
        "POF-Search",    ["Search"]
    )
    searchTitles := titleMap.Has(title) ? titleMap[title] : [title]
    for _, candidateTitle in searchTitles {
        if WinExist(candidateTitle " ahk_exe msedge.exe") {
            hwnd := WinGetID(candidateTitle " ahk_exe msedge.exe")
            HideFromTaskbar(hwnd)
            ApplyHtmlPanelState(hwnd)
            WinActivate(candidateTitle " ahk_exe msedge.exe")
            return
        }
    }


    ; Launch in Edge app mode (locked to Edge; never Chrome)
    browser := ""
    ...
    if browser = ""
        browser := "msedge.exe"
    Run(chr(34) . browser . chr(34) . " --app=" . url . " --window-size=420,750 --disable-extensions --disable-background-networking --disable-sync --disable-component-update --disable-features=msEdgeSidebarV2,msUndersideButton,EdgeShoppingAssistant --no-first-run --no-default-browser-check")
    ; After launching, poll for window and hide from taskbar (repeating timer, 500ms interval, 15s timeout)
    SetTimer(HideNewPanel.Bind(searchTitles, A_TickCount), 500)
}

HideNewPanel(searchTitles, startTick) {
    ...
    for , exe in ["chrome.exe", "msedge.exe"] {
        for _, candidateTitle in searchTitles {
            winTitle := candidateTitle " ahk_exe " exe
...
ApplyHtmlPanelState(hwnd) {
    ...
    defaultX := monRight - defaultW - 24
    defaultY := monTop + 24

    state := ""
    if FileExist(stateFile) {
        try {
            state := FileRead(stateFile, "UTF-8")
        } catch {
            state := ""
        }
    }
    ...
    hasSavedPos := false
    if (state != "") {
        if RegExMatch(state, '"x"\s*:\s*(-?\d+)', &mx)
            x := Integer(mx[1])
        if RegExMatch(state, '"y"\s*:\s*(-?\d+)', &my)
            y := Integer(my[1])
        if RegExMatch(state, '"w"\s*:\s*(\d+)', &mw)
            w := Integer(mw[1])
        if RegExMatch(state, '"h"\s*:\s*(\d+)', &mh)
            h := Integer(mh[1])
        hasSavedPos := RegExMatch(state, '"x"\s*:\s*(-?\d+)') && RegExMatch(state, '"y"\s*:\s*(-?\d+)')
    }

    ; Clamp saved positions back onto the current visible work area so
    ; browser app panels do not reopen off-screen after monitor/layout changes.
    if (w < 280)
        w := defaultW
    if (h < 220)
        h := defaultH
    maxW := monRight - monLeft
    maxH := monBottom - monTop
    if (w > maxW)
        w := maxW
    if (h > maxH)
        h := maxH
    ...
    if RegExMatch(state, '"pinned"\s*:\s*true') {
        try {
...
; Ctrl+Shift+Space = Quick action popup (select text first, popup appears at mouse)
^+Space:: {
    global gSelectedText
    oldClip := A_Clipboard
    A_Clipboard := ""
...
; Ctrl+Shift+P = Popup Hub (action launcher)
^+p:: {
    Run('"D:\DONT TOUCH BOOT UP\PopupHub\START_POPUP_HUB.bat"')
}
...
; â”€â”€ Quick-paste slots 1-10 (fed by the clipboard front end via slot_cache) â”€â”€
; The web UI writes each slot to a plain .txt; these hotkeys paste it, and work
; even if the front end is closed because they read the local cache file.
SlotCacheDir := A_ScriptDir "\slot_cache"

PasteSlot(n) {
    global SlotCacheDir
    f := SlotCacheDir "\slot_" n ".txt"
    if !FileExist(f) {
        ToolTip("Slot " n " is empty")
        SetTimer(() => ToolTip(), -800)
        return
    }
    A_Clipboard := FileRead(f, "UTF-8")
    if ClipWait(0.6)
        Send("^v")
}
...
RefreshSlots() {
    try {
        ...
        req.Open("GET", "http://127.0.0.1:3456/api/slots/refresh", false)
        req.Send()
    }
}
